...
rho_solvent = 0.0 # density of solvent g/mL
...
# sphericites, masses of solute and solvent, cavity radius
...
#rho_vdw = (m_solvent*1E30)/((100^3)*N_Av*vol_solvent)
...
fac = 1#*(psi_solvent)
#print(atoms_solvent)
#print("m_solvent = ", m_solvent)
#print("rho_vdw = ", rho_vdw)
print("rho_solvent = ", rho_solvent)
#print("vol_solvent = ", vol_solvent)
print("vol_free/vol_solvent = ", vol_free/vol_solvent)
rc = (vol_solute**(1/3) + fac*(vol_free**(1/3)))*((3/(4*pi))**(1/3))    

lx,ly,lz = max_rv(atoms_solute,x_solute,y_solute,z_solute)
#phi_solute = vol_solute/(8*lx*ly*lz)
phi_solute = area_solute/(4*(lx*ly+lx*lz+ly*lz))
print("phi_solute = ", phi_solute)
lx,ly,lz = max_rv(atoms_solvent,x_solvent,y_solvent,z_solvent)
#phi_solvent = vol_solvent/(8*lx*ly*lz)
phi_solvent = area_solvent/(4*(lx*ly+lx*lz+ly*lz))
print("phi_solvent = ", phi_solvent)
fac_phi = phi_solvent/phi_solute #1/(phi_solute/phi_solvent)
ax = (area_solute/area_solvent)*fac_phi
s_phi = ax
...
#        dipole_eff = dipole_solute
        dipole_int = 2*1E-30*dipole_eff*dipole_solvent*(3.335**2)/ \
                     (4*pi*epsilon_0*epsilon_r*(rc**3))
#      dipole_int = 2*1E-30*dipole_eff*dipole_solvent*(3.335**2)/ \
#                     (4*pi*epsilon_0*epsilon_r*vol_solute*3*m.sqrt(3)/8)
        kT = (k*T)*(1.60218E-19)
        ax = (area_solute/area_solvent)
...
#               m.exp(-0.5*dipole_int/kT)
        p_rot = m.exp(-rab/n_rot) * \
               m.exp(-0.5*dipole_int/kT)
        #p_rot = sigma_r*p_rot/(1+sigma_r*p_rot)
        p_not = 1-p_rot
        p_rot = sigma_r*p_rot/(p_not+sigma_r*p_rot)
...
#           p_rot = m.exp(-(1 - psi_solute)*rab/(n_rot*sigma_r))
            ...
            p_not = 1-p_rot
            p_rot = sigma_r*p_rot/(p_not+sigma_r*p_rot)
            #p_rot = sigma_r*p_rot/((1-m.exp(-rab/0.1))+sigma_r*p_rot)
...
psi_solvent = sphericity(vol_solvent,area_solvent)
xfac = 3*m.log((pi*m.sqrt(3)/6))/m.log(pi/6) #+ 3*m.log((pi*ax/6 + 2*m.sqrt(pi*ax/6))/((ax+2*m.sqrt(ax))))/m.log(pi/6)
#fac = ax
#xfac = (m.log(2*ax+1) - m.log(ax+2*m.sqrt(ax)))/(m.log(((3*ax-1)/2)**(2/3))-m.log(ax))
print("fac_phi = ", fac_phi)
#fac = fac*((psi_solute/psi_solvent)**xfac)
fac = (ax*fac_phi+2*m.sqrt(ax*fac_phi))/3
fac = ax*fac_phi
if (surface_tension):
