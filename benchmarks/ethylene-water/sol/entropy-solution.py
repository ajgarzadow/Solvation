...
omega = -1 # to be determined later if not defined
nu0 = 100
nu0_t = 100
nu0_r = 100
...
Pitzer = {"toluene" : 0.263, "water" : 0.344}
...
S_r = k*(0.5 + m.log(qr) - m.log(sigma_r)/3)
S_r_gas = 3*k*(0.5 + m.log(qr) - m.log(sigma_r)/3)
...
print(w, 3*S_v*23.06*1000, 3*S_r*23.06*1000)
if (interp_Sr):
    S_r = 3*(S_r*(psi_solute) + S_r_mu*(1 - psi_solute))
else:
    S_r = S_r_gas
...
w = 0
S_t = 3*k*(0.5 + m.log(qr) - m.log(sigma_r)/3)
...
print("Delta S_sol =  ", (S_tot - S_gas)*23.06*1000)
