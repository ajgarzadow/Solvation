...
f_k = 1/2 # kinetic energy of solvent molecules in a certain direction is f_k*kT
f_d = 1 # dipole moment interaction barrier coefficient
concentration = 1 # M
...
if (B_av == 0.):
    ...
    dipole_int = kirkwood_f*1E-30*tot_dipole_solute*tot_dipole_solvent*(3.335**2)/ \
                 (4*pi*epsilon_0*(rc**3))
    dipole_intX = kirkwood_f*1E-30*dipole_solute[0]*tot_dipole_solvent*(3.335**2)/ \
                 (4*pi*epsilon_0*(rc**3))
    dipole_intY = kirkwood_f*1E-30*dipole_solute[1]*tot_dipole_solvent*(3.335**2)/ \
                 (4*pi*epsilon_0*(rc**3))
    dipole_intZ = kirkwood_f*1E-30*dipole_solute[2]*tot_dipole_solvent*(3.335**2)/ \
                 (4*pi*epsilon_0*(rc**3))
    # MechElec model
    ...
    if (n_rot < 3):
        ...
        facX = facX*m.exp(-f_d*dipole_intY/kT)*m.exp(-f_d*dipole_intZ/kT)                
        facY = facY*m.exp(-f_d*dipole_intX/kT)*m.exp(-f_d*dipole_intZ/kT)                
        facZ = facZ*m.exp(-f_d*dipole_intY/kT)*m.exp(-f_d*dipole_intX/kT)        
        p_rot0 = (m.exp(-f_k*rab*rab/3)*facZ  + \
                  m.exp(-f_k*rab*rab/3)*facY + \
                  m.exp(-f_k*rab*rab/3)*facX )
        p_rot0 = m.exp(-f_k*rab/n_rot) * \
                 m.exp(-f_d*dipole_int/kT)
#    p_rot = sigma_r*p_rot0/(1+(sigma_r-1)*p_rot0)
    #fsr = min(sigma_r,1)
    #p_rot = fsr*p_rot0/(1+(fsr-1)*p_rot0)
    p_rot = p_rot0
...
#    p_rot_mech = sigma_r*p_rot0_mech/(1+(sigma_r-1)*p_rot0_mech)
    ...
    #p_rot_mech = fsr*p_rot0_mech/(1+(fsr-1)*p_rot0_mech)
...
if (solute_xyz == solvent_xyz):
    ...
    if (B_av2 == 0.):
        ...
        dipole_intX = kirkwood_f*1E-30*dipole_solvent[0]*tot_dipole_solvent*(3.335**2)/ \
                      (4*pi*epsilon_0*(rc2**3))
        dipole_intY = kirkwood_f*1E-30*dipole_solvent[1]*tot_dipole_solvent*(3.335**2)/ \
                      (4*pi*epsilon_0*(rc2**3))
        dipole_intZ = kirkwood_f*1E-30*dipole_solvent[2]*tot_dipole_solvent*(3.335**2)/ \
                      (4*pi*epsilon_0*(rc2**3))
        if (n_rot2 < 3):
            ...
            facY = facY*m.exp(-f_d*dipole_intX/kT)*m.exp(-f_d*dipole_intZ/kT)                
            facZ = facZ*m.exp(-f_d*dipole_intY/kT)*m.exp(-f_d*dipole_intX/kT)        
            p_rot0 = (m.exp(-f_k*rab/3)*facZ  + \
                  m.exp(-f_k*rab/3)*facY + \
                  m.exp(-f_k*rab/3)*facX )
            p_rot0 = m.exp(-f_k/n_rot2) * \
                     m.exp(-f_d*dipole_int2/kT)            
        #p_rot = sigma_r2*p_rot0/(1+(sigma_r2-1)*p_rot0)        
        p_rot = p_rot0
        print("tot_dipole_solvent = ",tot_dipole_solvent)
        ...
        #S_r_spt2 = S_r_spt2 + k*m.log(sigma_r2) 
...
#p_hop = max(pi*((vol_free*3/(4*pi))**(2/3)) - area_solute/3,0)/(pi*(vol_free*3/(4*pi))**(2/3))
fac_N = (area_solute/area_solvent)*fac_phi
#p_hop = max(vol_free*fac_N - vol_solute,0)/(fac_N*vol_free)
#p_hop = max((vol_free**(2/3))*fac_N - area_solute,0)/(fac_N*vol_free**(2/3))
#p_hop = p_hop**(2/3)
p_hop2 = (1/(1-p_hop)) - 1
#vc = vc*((1/(1-p_hop))**3)
vc = vc*(1 + 6*fac_N*p_hop2)
#vc = vc*(1/(1-p_hop))
#p_hop = max(vol_free*fac_N - vol_solute,0)/(vol_free*fac_N)
#p_hop = max((vol_free**(2/3))*fac_N - vol_solute**(2/3),0)/(vol_free**(2/3))
#p_hop = max((vol_free**(2/3))*fac_N - area_solute,0)/(fac_N*vol_free**(2/3))
#vc = vc*(1/(1-p_hop))
#vc = ((4*pi/3)*rc**3 - vol_solute)*((1E-10)**3)
#vc = 6*fac_phi*(area_solute/area_solvent)*vol_free*((1E-10)**3)
#rc = (6*fac_phi*(area_solute/area_solvent)*vol_free*((3/(4*pi))))**(1/3)    
#vc = fac_phi*(6*(area_solute/area_solvent)*vol_free-vol_solute)*((1E-10)**3)
#vc = ((vol_solute/vol_solvent)*vol_free)*((1E-10)**3)
...
#scal = (2**(1/3) - 1)**(-3) #2*m.sqrt(2)/pi
#scal =  (((vol_solvent+vol_free)/vol_free)**(1/3) - 1)**(-3)
scal =  1#13/9#2*3*m.sqrt(2)/pi
def SPT_G_cav(y,R):
    #y = y/scal; R = (scal**(1/3))*R
    G_cav = k*T*(-m.log(1-y)   + (R)*3*y/(1-y)  + \
                 (3*y/(1-y) + (9/2)*((y/(1-y))**2))*((R)**2))
    print("G_c terms = ",-m.log(1-y),(R)*3*y/(1-y), \
          3*y/(1-y)*(R**2), (9/2)*((y/(1-y))**2)*(R**2))
    #G_cav = G_cav + (y*P/((rho_m/(1E-30))))*(R**3)/(1.602E-19)
    return G_cav
# SPT cavity entropy
def SPT_S_cav(RM,RS,alpha_V):
    #RS = RS/scal; RM = RM/scal
    y0 = (pi/6)*((2*RS)**3)*rho_m; R = RM/RS
    #y = (3/(4*pi))*(epsilon_r - 1)/(epsilon_r + 2)
    if (epsilon_r == 1):
        y = (pi/6)*((2*RS)**3)*rho_m; R = RM/RS
    else:
        y = (3/(4*pi))*(epsilon_r - 1)/(epsilon_r + 2); R = RM/RS    
    print("y0, y, R, RS terms = ", y0, y, R, RS)
    RS = (1/2)*((y*6/pi)/rho_m)**(1/3)#; RM = RM*y/y0; R = RM/RS    
    #
    #/sphericity(vol_solute,area_solute)
    #y = y/scal; R = (scal**(1/3))*R    
    #RS = vol_solvent; R = RM/RS
    #y = vol_solvent*rho_m
    drho_dT = -rho_m*alpha_V; RC = RM + RS#/(scal**(1/3))
    #df_dy = -((R**2)*(-6*y+3)+3*R*(y-1)-((y-1)**2))/((1-y)**3)
    #df_dy = -((R**2)*(6*y+3)-3*R*(-1+y)+((-1+y)**2))/((-1+y)**3)
    df_dy = -(-3*R*(-1+y) + (-1+y)**2 +(R**2)*(3+6*y))/((-1+y)**3)
    #df_dy = df_dy + (1*P/((rho_m/(1E-30))))*(R**3)/(1.602E-19)
    #df_dy = 1/(1-y) #+ R*3/((1-y)**2)
    #dy_dT = (y/rho_m)*(-rho_m*alpha_V)
    dy_dT = -alpha_V*y    
    df_dR =  3*y/(1-y) + \
                 (3*y/(1-y) + (9/2)*((y/(1-y))**2))*(2*R)
    dR_dT = 0#-(1/3)*(RM/((RC - RM)**2))*vol_solvent*rho_vdw*alpha_V/\
            #(((4*pi/3)**(1/3))*rho_solvent*((scal**(2/3))*vol_free**(2/3)))
    #dR_dT = -(alpha_V/3)*(vol_solute**(1/3))*vol_solvent*rho_vdw/\
    #        (((scal*vol_free)**(4/3))*rho_solvent)
    #dR_dT = dR_dT*scal**(-2/3) 
...
def get_alpha(omegaX,RM,RS,alpha_V):
    ...
    #        (((scal*vol_free)**(4/3))*rho_solvent)
    #dR_dT = dR_dT*scal**(-2/3) 
    print("S_r_spt2, S_r_gas2 = ",S_r_spt2, S_r_gas2)
    #alpha_spt = (5.365*k*omegaX + S_r_spt2 - S_r_gas2 - SPT_G_cav(y,R)/T)/\
     #           (k*T*(df_dy*dy_dT + df_dR*dR_dT)/alpha_V)
    alpha_spt = -(-5.365*k*omegaX - DS_r0 + SPT_G_cav(y,R)/T)/\
                (-k*T*y*df_dy)
#    alpha_spt = ((min(0,5.365*k*omegaX + S_r_spt2 - S_r_gas2)) - SPT_G_cav(y,R)/T)/\
#                (k*T*(df_dy*dy_dT + df_dR*dR_dT)/alpha_V)    
    return alpha_spt
if (omega != None):
