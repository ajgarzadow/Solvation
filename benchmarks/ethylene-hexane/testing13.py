...
# Atomic masses 
mass = {"H" : 1.008, \
         "B": 10.811, "C" : 12.011, "N" : 14.007, "O" : 15.999, "F" : 18.998, "Ne" : 20.18, \
         "Al" : 26.98, "P" : 30.974, "Cl" : 35.453, "S" : 32.06, "Ar" : 39.948, \
         "Ni" : 58.690, "I" : 126.90, "Kr" : 83.8, \
         "Xe" : 131.29}
# Pitzer accentric factors (for future use)
Pitzer = {"toluene" : 0.263, "water" : 0.344, "iodine" : 0.229}
# Pyykko's covalent radii in pm (not used)
r_cov = {"H" : 32, \
         "C" : 75, "N" : 71, "O" : 63, \
         "P" : 111, \
         "Ni" : 110}
...
r_vdW = {"H" : 120, \
         "B" : 180, "C" : 170, "N" : 155, "O" : 152, "F" : 147, "Ne" : 154, \
         "Al" : 184, "P" : 180, "Cl" : 175, "S" : 180, "Ar" : 188, \
         "Ni" : 163, "I" : 198, "Kr" : 202, \
         "Xe" : 216}
...
if (dipole_solvent == None):
    ...
    dipole_solvent = 0
if ((dipole_solute == None) & (dipole_file == None)):
...
# Get dipole moment of solute if given in Gaussian file and not input
if (dipole_file != None):
    ...
    for line in f_dipole.readlines():
        ...
        if (len(aline) >= 8):
            if (aline[0] == 'X='):
                dipole_solute = float(aline[7])
    f_dipole.seek(0)
    f_dipole.close()
    print("dipole_solute = ", '%.2f'%dipole_solute)
...
if (solute_xyz == solvent_xyz):
    ...
    if (B_av2 == 0.):
        ...
        #p_rot = sigma_r2*p_rot0/(1+(sigma_r2)*p_rot0)
...
#print("For ECP:")
#print('%.2f'%DS, '%.2f'%DS_spt,'%.2f'%H_tot)
#print(" Delta H: ")
